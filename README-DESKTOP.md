...
FramePick 桌面版支持在同一个进程中打开多个项目窗口。在顶部原生菜单的“文件 → 新窗口”中创建一个独立的空白项目窗口，然后在“文件 → 打开项目”载入另一个 `.fpproj`。每个窗口都有自己的素材、序列帧、播放状态、撤销记录和保存路径；在某个窗口打开素材库、取帧工作区、序列帧或属性面板时，面板只属于该项目窗口，不会串到其他项目。

主项目窗口使用自定义标题栏显示 FramePick 品牌、当前项目名和保存状态；Windows 的最小化、最大化和关闭按钮仍保留在右上角。新建项目、新窗口、打开/保存、导入/导出和设置统一位于窗口顶部的原生菜单中。独立面板保留自己的系统标题栏，菜单命令会作用于它所属的项目窗口。
...
关闭项目窗口时，如果当前项目自上次保存后有新修改，会弹出“保存 / 不保存 / 取消”提示。选择“保存”会先完成完整项目快照，保存对话框取消或保存失败时窗口不会关闭；多个项目窗口分别判断自己的未保存状态。
...
- `transformed/`：按当前原始/AI 序列选择，只应用单帧自身的位移、缩放和旋转；整图节点动画作为独立曲线保存在 `sequence.json`。导出面板中的分辨率默认跟随项目画布，也可以自定义最终帧宽高；内容保持原像素比例并居中放入指定输出画布。

计算内容包围盒时会读取 Alpha 实际可见范围，并纳入每帧自身的位移、缩放和旋转。所有 `transformed/` 帧使用同一个联合裁切原点，因此会保留角色在不同帧之间的真实位移；输出画布尺寸由导出面板决定，尺寸大于内容时保留透明边缘，尺寸小于内容时按统一原点裁切，不会对每帧单独居中裁切。
